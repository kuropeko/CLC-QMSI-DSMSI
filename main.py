#streamlit run "D:\CODE COMPILATION\Company\main.py"
...
# --- CONFIGURATION ---
FORMS_DIR = r"D:\CODE COMPILATION\Company\FORMS"
TRACKER_FILE = r"D:\CODE COMPILATION\Company\usage_tracker.json"
...
def main_app():
    ...
    def get_all_folders():
        if not os.path.exists(FORMS_DIR):
            return []
        return sorted([d for d in os.listdir(FORMS_DIR) if os.path.isdir(os.path.join(FORMS_DIR, d))])
    ...
    with st.sidebar:
        ...
        with st.form("create_folder", clear_on_submit=True):
            new_folder_name = st.text_input("Folder name", placeholder="e.g. HR Documents")
        ...
        if folders:
            ...
            if uploaded and st.button("⬆️ Upload Files", use_container_width=True):
                ...
                for uf in uploaded:
                    ...
                    count += 1
                st.success(f"✅ {count} file(s) uploaded to **{upload_dest}**!")
                st.rerun()
        else:
        ...
        if folders:
            ...
            with col_r:
                with st.popover("✏️ Rename"):
                    ...
                    if st.button("Confirm Rename", key="rn_confirm"):
                        if rn.strip():
            ...
            with col_d:
                with st.popover("🗑️ Delete"):
                    ...
                    if st.button("Yes, Delete", key="del_folder_confirm"):
    ...
    st.caption("Organized Document Stamping System")
    ...
    with tab_browse:
        ...
        if not all_files:
            ...
            hc1, hc2, hc3, hc4 = st.columns([3, 2, 1, 1.8])
            ...
            for folder_name, pdf_name in all_files:
                ...
                c1, c2, c3, c4 = st.columns([3, 2, 1, 1.8])
                with c1:
                    st.write(f"📄 {pdf_name}")
                with c2:
